...
function post_processing(sol_gpu, V_mat, S, t_save, t_selected, ωd)
    ...
    rho_final_SW_mat = V_mat * rho_final_dressed_mat * V_mat'

    U_SW_inv = exp(Matrix(-S.data))
    rho_final_bare_mat = U_SW_inv * rho_final_SW_mat * U_SW_inv'
    #rho_final_bare_mat = rho_final_SW_mat
...
function H_num(p::SystemParams)
    ...
    H_2nd_filter = 0.5 * commutator(S, commutator(S, Hint_P))
    H_2nd = 0.5 * commutator(S, Hint)
    H_3rd = (1.0/3.0) * commutator(S, commutator(S, Hint))
    H_4th = (1.0/8.0) * commutator(S, commutator(S, commutator(S, Hint))) # The Kerr-shift terms!

    # Enforce strict Hermiticity to prevent floating-point drift over long times
    H_raw = H0 + Hint_P + H_1st_filter + H_2nd_filter + H_2nd + H_3rd + H_4th
...
F = 0.025
kp = 0.1 #TODO change this 
ωd = 1.985641  #1.983329 #1.985607 
...
# 10. Wigner function
rho_mode1_rotated, t_selected_idx = post_processing(sol_gpu, V_mat, S, t_save, tmax, ωd)
